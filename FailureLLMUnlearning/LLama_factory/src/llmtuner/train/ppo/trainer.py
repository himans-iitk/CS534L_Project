...
# Compatibility layer for trl.core (removed in newer trl versions)
try:
    from trl.core import PPODecorators, logprobs_from_logits
except ImportError:
    # Implement missing functions for newer trl versions
    import torch.nn.functional as F
    
    def logprobs_from_logits(logits, labels):
        """Compute log probabilities from logits and labels."""
        log_probs = F.log_softmax(logits, dim=-1)
        log_probs_labels = torch.gather(log_probs, dim=-1, index=labels.unsqueeze(-1))
        return log_probs_labels.squeeze(-1)
    
    class PPODecorators:
        """Decorator class for PPO training (compatibility layer)."""
        @staticmethod
        def empty_device_cache():
            """Decorator to empty device cache after function execution."""
            def decorator(func):
